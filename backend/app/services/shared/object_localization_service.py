...
def analyze_object_localization(
    ...
    """Google Cloud Vision Object Localization — named objects with
    bounding boxes ('Building', 'Person', 'Tree'). Scene-only signal."""
    ...
    try:
        ...
        raise RuntimeError(f"Cloud Vision Object Localization failed HTTP {exc.code}: {detail}") from exc
    except URLError as exc:
        raise RuntimeError(f"Cloud Vision Object Localization request failed: {exc}") from exc

    responses = raw_response.get("responses", [])
    if not responses:
        raise RuntimeError("Cloud Vision Object Localization returned no responses.")
    payload = responses[0]
    if "error" in payload:
        raise RuntimeError(f"Cloud Vision Object Localization failed: {payload['error']}")
    ...
    objects = [
        ...
        for item in payload.get("localizedObjectAnnotations", [])
    ]
